...
### 💡 [Bug Fix & UX 개선] 트렌딩 대시보드 출력 버그 수정 및 기본 탭 변경
**완료 시간**: 2026-04-25 11:28 KST

**1) 구현한 내용**
- **Trending 단일 레포만 노출되는 버그 수정**: `src/app/api/trending/route.ts` 및 크론잡(`fetch-hot`, `fetch-trending`)에서 `snapshotDate`를 비교할 때 밀리초 단위까지 정확히 비교하던 로직을 **날짜(YYYY-MM-DD)** 단위로 비교하도록 수정하여 필터링 버그를 완벽히 해결했습니다. 크론잡에서는 저장 시 자정(00:00:00.000)으로 정규화하도록 강제했습니다.
- **기본 탭(UX) 변경**: 사용자가 페이지에 진입했을 때 먼저 노출되는 탭을 `🔥 Hot`에서 `✨ Rising`으로 변경했습니다. URL 쿼리 파라미터 로직과 컴포넌트의 토글 순서를 역순(Rising → Hot)으로 배치했습니다.

**2) 왜 이 방법을 선택했는가? (근거)**
- JavaScript의 `new Date().toISOString()`은 고유한 밀리초를 반환하므로, Prisma를 통해 저장되는 `snapshotDate`가 루프를 돌 때마다 미세하게 달랐습니다. 따라서 DB 조회 후 필터링을 할 때 첫 번째 인덱스의 Date와 완전히 일치하는 요소(1개)만 남는 치명적 결함이 발생했습니다. 근본적인 원인을 DB 저장 단계와 API 필터 단계 양방향에서 차단했습니다.
- "신규 레포(Rising)"에 대한 노출 빈도를 높이고자 하는 UX 기획 변경에 맞춰, 프론트 상태값 및 API 기본 `source` 값을 `rising`으로 일치시켰습니다.
